#![cfg(windows)]
//! Validates that `checkout_service_gui` (the iced GUI demo target) produces
...
fn gui_target_path() -> std::path::PathBuf {
    ...
    target_debug.join("examples").join("checkout_service_gui.exe")
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn checkout_service_gui_matches_console_topology() {
    let target = gui_target_path();
    if !target.exists() {
        panic!(
            "checkout_service_gui.exe not found at {:?}.\nBuild it first: cargo build --example checkout_service_gui -p heaplens-alloc",
            target
        );
    }
    ...
    match tokio::time::timeout(Duration::from_secs(45), graph_handle).await {
        Ok(Ok(())) => {}
